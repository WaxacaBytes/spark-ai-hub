...
sah <client> --install  # make the plain client use the Hub when Mode is persistent
sah <client> --status   # show the client's Spark AI Hub wiring status
sah <client> --restore  # restore previous plain client settings when Mode is persistent
...
leaves the plain client untouched. `sah integrations` shows each client's
Mode. For `persistent` clients, `sah <client> --install` backs up the original
settings and rewrites them so the plain client uses the Hub until
`sah <client> --restore` puts the original settings back. `launch only`
clients support `sah <client>` but do not have persistent install/restore.
