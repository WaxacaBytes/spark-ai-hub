...
**One-liner:** SparkForge is the missing app store for DGX Spark — a web-based launcher that turns community-verified Docker recipes into one-click AI workloads.
...
**SparkForge unifies all of this** into a browsable catalog with one-click deploy.
...
│                                                          │
│  ┌────────────────────────────────────────────────────┐  │
│  │         SparkForge Daemon (Python/FastAPI)          │  │
│  │                                                     │  │
│  │  ┌─────────────┐  ┌──────────────┐  ┌───────────┐ │  │
...
**P2.3 — Model Manager**
- Shared model storage directory (`~/sparkforge/models/`)
...
## 6. Installation of SparkForge Itself

The tool must be trivially installable. One command:

```bash
curl -fsSL https://sparkforge.dev/install.sh | bash
```

What this does:
1. Creates `~/sparkforge/` directory
2. Clones the SparkForge daemon + frontend (pre-built)
3. Clones the recipe registry
4. Installs a systemd service (`sparkforge.service`)
5. Starts the service on port 9000
6. Prints: "SparkForge is running at http://localhost:9000"
...
├── sparkforge.service               # systemd unit
└── README.md
```
...
| Feature | Pinokio | NVIDIA Sync | NVIDIA Playbooks | SparkForge |
|---------|---------|-------------|-----------------|------------|
| Works on DGX Spark | ❌ | ✅ | ✅ | ✅ |
...
- **SparkForge** — Spark (DGX Spark) + Forge (building/crafting). Strong, memorable.
