...
from src.MetaView_dit import MetaViewSelfAttention3D

class MetaViewTrainingModule(torch.nn.Module):
    def __init__(self):
        super().__init__()
    ...
    def switch_pipe_to_training_mode(
        ...
        if "prope_attn" in trainable_models:
            pipe.eval()
            pipe.requires_grad_(False)
            cnt = 0
            for name, module in pipe.dit.named_modules():
                if isinstance(module, (MetaViewSelfAttention3D)):
                    module.train()
                    module.requires_grad_(True)
                    cnt += 1
                    print(name)
                if "3D" in name.split(".")[0]:  # _3D_in, proj_3D_out
                    module.train()
                    module.requires_grad_(True)
                    print(name) 
                    
            print(f"{cnt} PRoPE Attention layers are trainable. ")
        else:
